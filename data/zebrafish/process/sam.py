...
def count_metrics(sam_file):
    ...
    for rname, positions in metrics_count.items():
        pos_metrics = {"PS": [], "RD": [], "ED": [], "MC": []}
        for bp, counts in sorted(positions.items()):
            pos_metrics["PS"].append(bp)
            pos_metrics["RD"].append(counts["RD"])
            pos_metrics["ED"].append(counts["ED"])
            pos_metrics["MC"].append(counts["MC"])
        metrics_data[rname] = pos_metrics
