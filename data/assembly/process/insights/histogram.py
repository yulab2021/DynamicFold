...
for feature in tqdm(features, desc="Calculate Features"):
    ...
    if logarithmic:
        data = [np.log10(abs(float(row[0] * scale_factor)) + eps) for row in cursor.fetchall()]
    else:
        data = [float(row[0] * scale_factor) for row in cursor.fetchall()]
    plt.figure(figsize=(10, 5), dpi=300)
    plt.hist(data, bins=num_bins, edgecolor="white")
    if logarithmic:
        plt.xlabel(f"$\\log_{{10}}$(| {scale_factor} {feature} | + {eps})")
    else:
        plt.xlabel(feature)
