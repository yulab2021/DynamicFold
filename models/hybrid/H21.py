...
class H21(nn.Module):
    def __init__(self, network_depth, bottleneck_layers, input_size, max_len, seq_channels, map_channels, map_rank, kernel_size, pool_size, channel_rate, num_heads, temperature, activation, dropout):
        ...
        out_channels = in_channels * channel_rate
        self.map_ = Map(in_channels, map_channels, max_len, map_rank)
        self.bottleneck = BottleneckLayer(map_channels, bottleneck_layers, activation, num_heads, dropout)
        self.seq = Seq(in_channels, map_channels, out_channels, temperature, map_rank)
        in_channels = out_channels
...
class Seq(nn.Module):
    def __init__(self, seq_channels, map_channels, out_channels, temperature, map_rank):
        ...
        self.linear = nn.Linear(map_channels, out_channels)
    ...
    def forward(self, map_, seq):
        ...
        score = self.linear(torch.sum(value * attn, dim=2))
        return score
    
class Map(nn.Module):
    def __init__(self, seq_channels, map_channels, max_len, map_rank):
...
if __name__ == "__main__":
    model = H21(3, 3, 7, 4096, 64, 64, 64, 3, 4, 2, 16, 1, "GELU", 0.1).to(torch.device("cuda:1"))
    x = torch.randn(16, 7, 4000).to(torch.device("cuda:1"))
    y = model(x)
    print(y.shape)
