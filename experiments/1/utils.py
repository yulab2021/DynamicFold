...
class RNADataset(utils.Dataset):
    ...
    def __getitem__(self, index:int):
        key = self.keys[index]
        X = torch.tensor(self.Xs[index], dtype=torch.float32).to(dm.device)
        y = torch.tensor(self.ys[index], dtype=torch.float32).to(dm.device)
        return key, X, y
        ...
        with torch.no_grad():
            for keys, inputs, labels in tqdm(self.dataset.dataloaders[dataset_type], desc=f"Testing [{dataset_type} Set]"):
                ...
                for key, label, output in zip(keys, labels, outputs):
                    ...
                    results["Predictions"].append(orjson.dumps(output.cpu().tolist()).decode())
