...
class ConvLayerNorm1D(nn.Module):
    def __init__(self, num_channels):
        super(ConvLayerNorm1D, self).__init__()
...
class ConvLayerNorm2D(nn.Module):
    def __init__(self, num_channels):
        super(ConvLayerNorm2D, self).__init__()
        self.layer = nn.Sequential(
            Permutation((0, 2, 3, 1)),
            nn.LayerNorm(num_channels),
            Permutation((0, 3, 1, 2))
...
class Embedding1D(nn.Module):
    def __init__(self, input_size, output_size, embedding="linear", hidden_size=None, activation=None, encoding=None, max_len=None):
        ...
        if embedding == "MLP":
            ...
            norm = ConvLayerNorm1D(output_size)
