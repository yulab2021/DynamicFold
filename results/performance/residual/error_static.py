...
for _, row in data.iterrows():
    ...
    static_prediction.extend(orjson.loads(row["RibonanzaNetPredictions"]))

indices = list(range(len(dynamic_error)))
indices = np.random.choice(indices, size=bootstrap_size, replace=True)
static_sample = np.array(static_prediction)[indices]
error_sample = np.array(dynamic_error)[indices]

plt.figure(figsize=(6, 5), dpi=300)
sns.kdeplot(x=static_sample, y=error_sample, fill=True, levels=levels, cbar=True, cmap=sns.color_palette("Blues", as_cmap=True), gridsize=grid_size)
plt.xlim((0, 1))
plt.ylim((-1, 1))
plt.xlabel("RibonanzaNet Predicted Reactivity")
plt.ylabel("DynamicFold Error")
plt.title("Distribution of RibonanzaNet Prediction vs. Error per Base")
