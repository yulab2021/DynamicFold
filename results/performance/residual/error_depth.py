...
sns.kdeplot(x=read_depth_sample, y=error_sample, fill=True, levels=levels, cbar=True, cmap=sns.color_palette("Blues", as_cmap=True), gridsize=grid_size)
plt.xlim((0, 1))
plt.ylim((-1, 1))
plt.xlabel("Winsorize-Scaled Read Depth")
plt.ylabel("DynamicFold Error")
plt.title("Distribution of Read Depth vs. Error per Base")
