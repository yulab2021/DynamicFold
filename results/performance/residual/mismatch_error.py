...
sns.kdeplot(x=mismatch_rate_sample, y=error_sample, fill=True, levels=levels, cbar=True, cmap=sns.color_palette("Blues", as_cmap=True), gridsize=grid_size)
plt.xlim((np.log10(eps), 0))
plt.ylim((-1, 1))
plt.xlabel(f"$\\log_{{10}}$(Mismatch Rate + {eps})")
plt.ylabel("DynamicFold Error")
plt.title("Distribution of Mismatch Rate vs. Error per Base")
